...
class tx_hackRF(gr.top_block):

    def __init__(self, filename="samples/HT/beacon_mcs0_lgi.raw", freq=2412000000, gain=20):
...
def main(top_block_cls=tx_hackRF, options=None):
    if options is None:
        options = argument_parser().parse_args()
    tb = top_block_cls(filename=options.filename, freq=options.freq, gain=options.gain)
